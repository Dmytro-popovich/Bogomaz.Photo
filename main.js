...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ===== SECTION FOCUS OBSERVER =====
  const focusSections = document.querySelectorAll('.focus-section');
  const sectionObserverOptions = {
    // threshold: 0.1 works better for very tall sections on mobile
    threshold: [0.05, 0.1, 0.2, 0.3],
    rootMargin: window.innerWidth <= 768 ? '-10% 0px -10% 0px' : '-5% 0px -5% 0px'
  };

  const sectionObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      // On mobile we check if even a small part is visible in our sensitive zone
      const isVisible = entry.intersectionRatio > (window.innerWidth <= 768 ? 0.05 : 0.2);
